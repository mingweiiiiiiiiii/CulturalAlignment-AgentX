...
from llm_clients import get_llm_client, LLMResponse, format_chat_message
...
def test_llm_response_formatting():
    response = LLMResponse(
        content="Test content", role="assistant", metadata={"temperature": 0.7}
    )
...
def test_chat_message_formatting():
    message = format_chat_message(role="user", content="Test message", name="TestUser")
...
def test_llm_retries(mock_llm_client):
    ...
    responses = [Exception("First failure"), Exception("Second failure"), "Success"]
...
def test_llm_temperature_settings():
    ...
    creative_response = manager.generate("Test prompt", temperature=0.8)
    assert isinstance(creative_response, str)

    precise_response = manager.generate("Test prompt", temperature=0.2)
...
def test_concurrent_llm_requests(mock_llm_client):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = []
        for _ in range(5):
            futures.append(executor.submit(manager.generate, "Test prompt"))
